...
import { getCreditPageData }  from "@/lib/shop-cache";
...
export default async function CreditPage({ params }: Props) {
  ...
  const { credits, stats } = await getCreditPageData(shopId);

  return <CreditView activeShop={{ id: shop.id, name: shop.name, location: shop.location }} isAdmin={isAdmin} isManager={isManager} credits={credits} stats={stats} />;
}
