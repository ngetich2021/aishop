...
import { getAssetsPageData }  from "@/lib/shop-cache";
...
export default async function AssetsPage({ params }: Props) {
  ...
  const { assets, stats } = await getAssetsPageData(shopId);

  return <AssetsView activeShop={{ id: shop.id, name: shop.name, location: shop.location }} isAdmin={isAdmin} isManager={isManager} assets={assets} stats={stats} />;
}
